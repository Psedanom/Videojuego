...
AFTER UPDATE ON matchgame
FOR EACH ROW
BEGIN
    IF NEW.result = 'victory' THEN
        UPDATE player SET score = score + 1 WHERE idPlayer = NEW.idPlayer;
    END IF;
END$$
DELIMITER ;
...
FOR EACH ROW
BEGIN
    UPDATE player SET lastUpdate = NOW() WHERE idPlayer = NEW.idPlayer;
END$$
DELIMITER ;
...
BEGIN
    SELECT username, name, suit, e.name as effect_name
    FROM player
    INNER JOIN matchgame USING (idPlayer)
    INNER JOIN Deck d USING (idDeck)
    INNER JOIN cardindeck USING (idDeck)
...
BEGIN
    ...
    SELECT AVG(score) INTO average_score FROM player;
END$$
...
BEGIN
    ...
    SELECT AVG(idDeck) INTO average_usage FROM matchgame;
END$$
delimiter ;