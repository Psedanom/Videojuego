...
class CardEnemie extends Cards {
    ...
    draw(ctx) {
        ...
        ctx.drawImage(imgPicas, this.x - offsetX, this.y - offsetY,
            this.width * this.scale, this.height * this.scale);
        ctx.fillStyle = "white";
        ctx.font = "20px Arial";
        ctx.textAlign = "center";
        ctx.fillText(this.number, this.x - offsetX + 20, this.y - offsetY + 20);
    }
    actionUse(health) {
        health -= this.number;
        return health;
...
class CardVida extends Cards {
    ...
    draw(ctx) {
        let offsetX = (this.width * this.scale - this.width) / 2;
        let offsetY = (this.height * this.scale - this.height) / 2;
        ctx.drawImage(imgCorazon, this.x - offsetX, this.y - offsetY,
            this.width * this.scale, this.height * this.scale);
        ctx.fillStyle = "white";
        ctx.font = "20px Arial";
        ctx.textAlign = "center";
...
class CardEspada extends Cards {
    ...
    draw(ctx) {
        ...
        let offsetY = (this.height * this.scale - this.height) / 2;
        if (this.habilidad != "") {
            ctx.drawImage(imgRombos, this.x - offsetX, this.y - offsetY,
                this.width * this.scale, this.height * this.scale);
            ctx.fillStyle = "white";
            ctx.font = "20px Arial";
            ctx.textAlign = "center";
            ctx.fillText(this.number, this.x - offsetX + 20, this.y - offsetY + 20);
            ctx.font = "10px Arial";
            ctx.fillText(this.habilidad, this.x - offsetX + 50, this.y - offsetY + 100);
        } else {
            ctx.drawImage(imgRombos, this.x - offsetX, this.y - offsetY,
                this.width * this.scale, this.height * this.scale);
            ctx.fillStyle = "white";
...
class Game {
    ...
    newLevel(victory) {
        ...
        if (!victory && this.dificultad == 1.1) {
            this.cartas = [];
            for (let i = 1; i < 11; i++) {
